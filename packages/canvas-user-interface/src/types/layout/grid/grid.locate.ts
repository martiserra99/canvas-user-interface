...
export default function(grid: LayoutType) {
  grid.lifecycle.set('onLocate', function(grid: Layout, coords: Coords) {
    const contentCoords = {
  ...
  grid.inner.fun('getGridCoords', function(grid: Layout) {
    const x = grid.inner.call('getGridX');
    const y = grid.inner.call('getGridY');
    return { x, y };
  });

  grid.inner.fun('getGridX', function(grid: Layout) {
  ...
  grid.inner.fun('getGridY', function(grid: Layout) {
    const contentCoords = grid.inner.get('contentCoords');
    const contentSize = grid.inner.get('contentSize');
    const gridSize = grid.inner.get('gridSize');
  ...
  grid.inner.fun('getColumnsCoords', function(grid: Layout) {
  ...
  grid.inner.fun('getRowsCoords', function(grid: Layout) {
  ...
  grid.inner.fun('getPositionsCoords', function(
    _: Layout,
    gridCoord: number,
    positionsLengths: number[],
    gap: number
  ) {
    const coords = [];
    let coord = gridCoord;
  ...
  grid.lifecycle.set('getChildCoords', function(
    grid: Layout,
    _: Coords,
    child: Element
  ) {
  ...
  grid.inner.fun('getChildX', function(grid: Layout, child: Element) {
    ...
    const cellsLength =
      columnsWidths
        ...
        .reduce((acc: number, val: number) => acc + val + gap, 0) - gap;
  ...
  grid.inner.fun('getChildY', function(grid: Layout, child: Element) {
