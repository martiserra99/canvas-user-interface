...
export default function(grid: LayoutType) {
  grid.lifecycle.set('onMeasure', function(grid: Layout, maxSize: Size) {
  ...
  grid.inner.fun('getColumnsWidths', function(grid: Layout) {
  ...
  grid.inner.fun('getRowsHeights', function(grid: Layout) {
    const positions = grid.get('dimensions').rows;
    const availableContentLength = grid.inner.get('availableContentSize')
      .height;
  ...
  grid.inner.fun('getPositionsLengths', function(
    _: Layout,
    positions: { count: number; unit: string; value: number }[],
    availableContentLength: number,
    gap: number
  ) {
  ...
  grid.inner.fun('getGridSize', function(grid: Layout) {
    ...
    const sumColumnWidths = columnsWidths.reduce(
      (acc: number, val: number) => acc + val,
      0
    );
    const sumRowHeights = rowsHeights.reduce(
      (acc: number, val: number) => acc + val,
      0
    );
  ...
  grid.lifecycle.set('getChildMaxSize', function(
    grid: Layout,
    _: Size,
    child: Element
  ) {
    ...
    const width =
      columnsWidths
        ...
        .reduce((acc: number, val: number) => acc + val + gap.horizontal, 0) -
      gap.horizontal -
    ...
    const height =
      rowsHeights
        ...
        .reduce((acc: number, val: number) => acc + val + gap.vertical, 0) -
      gap.vertical -
  ...
  grid.lifecycle.set('getSize', function(grid: Layout, maxSize: Size) {
